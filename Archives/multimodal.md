...
6. 时间序列同时包含**可观测的显性特征**与**不可观测的隐性特征**，且二者在**多尺度（global–local）**上以不同方式作用于数据。（1）**显性特征**（如趋势方向/斜率、季节性周期与幅度、波动率水平、均值突变、异常尖峰等）往往能由传感器或规则化统计量直接刻画；**隐性特征**（如未记录的外生冲击、制度/政策变更、机理级状态跃迁、微气候/个体异质性、行为策略切换等）则难以观测或标注，只能通过模型在潜空间中加以解释。例如，在气候数据分析中，温度和湿度等属性是容易观测、定义清晰的；但 大气压变化 或 局地小气候 这类属性往往难以直接观测或准确定义。
（2）这些特征还呈现**不同尺度的影响模式**：趋势与长期季节性是**全局**作用，约束整个轨迹的形态与相位；周内/昼夜节律、波动团簇与局部均值漂移是**局部**或**中尺度**作用；尖峰、缺口和短暂冲击则是**瞬时**作用。更具挑战的是，特征间常**耦合与可替代**：相同外观可由不同潜在机制造成（可辨识性问题），局部编辑会沿因果/动力学路径产生**非局部回响**（ripple effects），而训练数据在属性组合空间里常**覆盖不均**，使编辑时频繁落入“组合外”（compositional OOD）区域。
由此，TSE 需要同时解决：**（1）识别与分离**——在缺乏完备标注的情形下区分显性与隐性成分并在多尺度上解耦；**（2）定位与对齐**——把自然语言或属性目标精确映射到对应时间片段/频段/结构分量；**（3）强度与守恒**——在历史保真与目标编辑之间动态权衡，既改到位又不破坏未指示的属性（editability–preservability）；**（4）不确定性传播**——在隐性因素与覆盖缺口存在时，给出可信的概率区间与风险评估。正是“**显性/隐性并存 + 全局/局部异质 + 组合覆盖不足 + 动力学耦合**”的叠加，构成了时间序列编辑的核心难点。
